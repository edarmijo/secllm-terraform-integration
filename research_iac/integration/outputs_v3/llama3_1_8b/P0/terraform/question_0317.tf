...
resource "aws_acm_certificate_validation" "video_cert" {
  ...
  validation_record_ids   = [aws_route53_record.video_cert.id]
  validation_method       = aws_acm_certificate.video_cert.validation_method
  validation_status       = "ISSUED"
  certificate_domain_validated = true

  depends_on = [
    aws_route53_record.video_cert,
  ]
}
