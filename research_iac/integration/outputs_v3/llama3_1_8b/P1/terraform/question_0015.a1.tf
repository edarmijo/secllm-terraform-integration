...
resource "aws_db_instance" "prod_db" {
  ...
  password             = aws_secretsmanager_secret_version.prod_db_password.secret_string
...
resource "aws_secretsmanager_secret_version" "staging_db_password" {
  secret_id     = aws_secretsmanager_secret.staging_db_password.id
  secret_string = var.staging_db_password
}
...
resource "aws_db_instance" "staging_db" {
  ...
  password             = aws_secretsmanager_secret_version.staging_db_password.secret_string
...
resource "aws_elastic_beanstalk_environment" "prod_env" {
  ...
  version_label       = "v1"
  environment_name    = aws_elastic_beanstalk_environment.prod_env.id
}
...
resource "aws_elastic_beanstalk_environment" "staging_env" {
  ...
  tier                = "webserver-medium"
  version_label       = "v1"
  environment_name    = aws_elastic_beanstalk_environment.staging_env.id
}