...
resource "aws_cloudfront_origin_access_control" "video_streaming_oac" {
  ...
  origin_access_control_policy   = jsonencode({
    Rules = [
      {
        Name = "Allow CloudFront to access S3 bucket"

        Options = {
          "CloudFront-Allowed-Origin": ["${aws_s3_bucket.video_streaming.bucket_domain_name}"]
        }
      }
    ]
  })
...
resource "aws_cloudfront_distribution" "video_streaming_cfd" {
  ...
  viewer_certificate {
    acm_certificate_arn            = aws_acm_certificate.video_streaming_cert.arn
