...
class ScriptPreprocessor:
    ...
    def _loadScript(self, file_path):
        ...
        pattern = r'(attribute\s+:[a-zA-Z_]*(?:mime|type|ext|format)[a-zA-Z_]*,\s*kind_of:\s*Array),\s*default:\s*\[[^\]]*\]'

        result = re.sub(pattern, r'\1', script_content, flags=re.IGNORECASE)

        processed_script = result
        #print("\nDOPO==>")
        #print(result)
    ...
    def preprocess(self, smell_name, scriptName, script):
        ...
        if prefilterRegEx:
            if smell['onlyCheckRegExpr'] == 'No':
                ...
                if smell['insideRegEx'] == 'Yes':
                    script = self.extractComm(script)
