...
if [ -z "$1" ] || [ -z "$2" ]; then
  echo "Usage: $0 <directory> <output_file> [config] [smell]"
  exit 1
fi
...
directory="$1"
output_file="$2"
config="$3" # Third argument captured the config file
smell="$4"  # Fourth argument captured as a single string, spaces included
...
if [ -n "$config" ]; then
  echo "Config argument passed: '$config'"
else
  echo "No config argument provided."
  config="config.yaml"
fi
...
for file in $files; do
  ...
  # Build the command
  if $first; then
    poetry run python ./secllm/secllm.py -f "$file" -o "$output_file" ${smell:+--smell "$smell"} --json --threads 3 --config "$config"
    first=false
  else
    poetry run python ./secllm/secllm.py -f "$file" -o "$output_file" --append ${smell:+--smell "$smell"} --json --threads 3 --config "$config"
  fi
done
