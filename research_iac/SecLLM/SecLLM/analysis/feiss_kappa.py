...
def load_data(files):
    """
    Load the CSV/JSON files containing the annotations (predictions) and combine them into a single DataFrame.
    Each file will contribute its annotations under the same PATH and LINE.
    
    Parameters:
        files (list): List of CSV/JSON file paths containing annotations.
...
def main():
    ...
    parser.add_argument('files', nargs='*', help="CSV or JSON files containing predictions with columns PATH, LINE, SMELL", type=str)
